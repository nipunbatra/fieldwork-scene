...
archive_dir=base/'native-archives';archive_dir.mkdir(exist_ok=True)
def digest(file):
 h=hashlib.sha256()
...
for district in sorted({c['district'] for c in selected}):
 ...
 if not archive.exists():
  url='https://github.com/nipunbatra/fieldwork-scene/releases/download/crop-burn-500-2026-09-10/'+archive.name
